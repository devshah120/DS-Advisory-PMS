...
import { clientsApi, parseApiError, type CreateClientInput } from '@/lib/clients.api';
...
const initial = {
  ...
  accountingMethod: null as AccountingMethod | null,
  feeRatePercent: '',
  inceptionDate: '',
  currency: 'USD',
  notes: '',
...
export default function AddClientPage() {
  ...
  const submit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      setErrors(fields);
      toast({ tone: 'error', title: 'Could not create client', description: message });
      setLoading(false);
    }
  };
        ...
        <form onSubmit={submit} noValidate className="space-y-6">
          ...
          <Card>
            <CardHeader
              ...
              subtitle="How this client’s XIRR is calculated. This cannot be guessed — pick the one that matches the records you actually receive."
            />
              ...
              {METHODS.map((m) => {
                ...
                return (
                  <button
                    key={m.value}
                    type="button"
                    onClick={() => set('accountingMethod', m.value)}
